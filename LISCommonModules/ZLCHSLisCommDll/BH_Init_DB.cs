...
namespace ZLCHSLisComm
{
    public class BH_Init_DB
    {
        ...
        string execution_time = null;//ini文件执行时间的结果值
        string FileName;   //ini文件的路径
        string[] mechanism_time = null;//得到ini文件中runtime的数组,以逗号间隔，
        string[] runtime = null;//机构|时间
        Double blanktime;//间隔时间
        string jigou = null;//机构
        ...
        public Double ReadRuntime()
        {
            try
            {
                ...
                if (!String.IsNullOrEmpty(Return_Value)) writeLog.Write(DateTime.Now.ToString() + "返回SOLVESET.INI文件中执行时间的结果值：" + Return_Value, "log");
                ...
                if (string.IsNullOrEmpty(execution_time)) return 0;
        ...
        public void Init(string yqid)
        {
            ...
            else if (databasetype == "2")
            {
                ...
                //else
                //    conStr = @"Persist Security Info=False;User ID=" + username + ";Password=" + password + ";Initial Catalog=" + databasename + ";Data Source=" + servername;
