    ...
    public class ResolveResult : IDataResolve
    {
        ...
        public void ParseResult(string strSource, ref string strResult, ref string strReserved, ref string strCmd)
        {
            ...
            try
            {
                ...
                string file = "SH210A样本号重复处理.txt";
                StreamReader sr = new StreamReader(@".\SH210A.txt", System.Text.Encoding.GetEncoding("GB2312"));
                ...
                if (!File.Exists(configPath))
                    return;
                string connStr = "Provider=Microsoft.Jet.OLEDB.4.0;Data Source=" + configPath + ";Persist Security Info=False;Jet OLEDB:Database Password=" + password;
                string sql = "select distinct(编号) as ItemNo  from  病人数据表 where Format(clrq, 'yyyy-mm-dd')='" + dateTime + "'";
