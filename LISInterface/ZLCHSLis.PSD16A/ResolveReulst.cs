    ...
    public class ResolveResult : IDataResolve
    {
        ...
        public void ParseResult(string strSource, ref string strResult, ref string strReserved, ref string strCmd)
        {
            try
            {
                ...
                StreamReader sr = new StreamReader(@".\PSD16A.txt");
                ...
                strSource = strSource.Replace(""+(char)0,"" );
                string ChannelCode = "";
                ...
                strTestTime = ss[0].Trim()+" "+ss[1].Trim();
                writelog.Write(strDevice, "  检验时间：" + strTestTime + "\r\n", "log");
                strSampleNo = ss[2];
                writelog.Write(strDevice, "  样本号：" + strSampleNo + "\r\n", "log");
                for (int i = 0; i < ss.Length; i++)
                {
