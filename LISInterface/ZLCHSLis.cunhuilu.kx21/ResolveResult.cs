...
namespace ZLCHSLis.cunhuilu.kx21
{
    public class ResolveResult : IDataResolve
    {
        ...
        public void ParseResult(string strSource, ref string strResult, ref string strReserved, ref string strCmd)
        {
            ...
            try
            {
                ...
                string file = "KX21样本号重复处理.txt";
                //读取配置文件确定txt文件的路径
                StreamReader configPathReader = new StreamReader(".\\KX21.txt", Encoding.Default);
                string correspondence = configPathReader.ReadLine();
                string configPath = configPathReader.ReadLine();
                configPathReader.Close();

                string fileName = configPath + DateTime.Now.ToString("yyyyMMdd").Substring(2, 6);
                fileName += ".txt";

                if (!File.Exists(fileName))
                    return;
                StreamReader srValues = new StreamReader(fileName, Encoding.Default);

                string readAllText = srValues.ReadToEnd().Trim();
                srValues.Close();

                DataTable tempData = new DataTable("Test");

                tempData.Columns.Add("itemNo", typeof(string));
                tempData.Columns.Add("itemName", typeof(string));
                tempData.Columns.Add("itemValue", typeof(string));

                List<string> allTexts = new List<string>();
                foreach (string lineText in readAllText.Split('\n'))
                {
                    if (lineText.IndexOf("接口号") > -1 && lineText.Split(new char[] { '：', ' ', ':', '\r', '\t' }, StringSplitOptions.RemoveEmptyEntries
    ).Length >= 6)
                    {
                        DataRow dr = tempData.NewRow();
                        string[] line = lineText.Split(new char[] { ' ', ':', '\r', '\t', '：' }, StringSplitOptions.RemoveEmptyEntries);
                        dr["itemNo"] = line[1].ToUpper();
                        dr["itemName"] = line[4].ToUpper();
                        dr["itemValue"] = line[5].Trim();
                        tempData.Rows.Add(dr);
                    }

                }
                DataView dataView = tempData.DefaultView;
                DataTable dataTableDistinct = dataView.ToTable(true, "itemNo");
               
                for (int outerI = 0; outerI < dataTableDistinct.Rows.Count; outerI++)
                {
                    string str = "";
                    DataRow[] rows = tempData.Select("itemNo='" + dataTableDistinct.Rows[outerI][0] + "'");
                    strSampleNo = dataTableDistinct.Rows[outerI][0].ToString();
                    List<string> TestValues = new List<string>();
                    foreach (DataRow item in rows)
                    {
                        str += item["itemName"] + "," + item["itemValue"] + "|";
                        TestValues.Add(item["itemName"] + ":" + item["itemValue"]);
                    }
                    ...
                    else
                    {
                        ...
                        for (int i1 = 0; i1 < strs.Length; i1++)
                        {
                            ...
                            FindRow = tItemChannel.Select("通道编码='" + strs[i1].Split(',')[0] + "'");
                            if (FindRow.Length == 0) //无普通结果则查找图像能道，无图像通道则更新通道类型为空
                            {
                                ChannelType = null;
                                //writelog.Write(strDevice, "未设置通道：" + strs[i1].Split(',')[0], "log");
                            }
                            else
                            {
                                testItemID = FindRow[0]["项目id"].ToString();
                                ChannelType = "0"; //普通结果
                                TestResultValue = TestResultValue + testItemID + "^" + strs[i1].Split(',')[1].Replace("RuPT", "阴性") + "|";
                            }
