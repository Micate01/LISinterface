...
namespace ZLCHSLis.Dirui.BF6800
{
    public class ResolveResult : IDataResolve
    {
        ...
        {
          string  CR = Convert.ToChar(13)+"";//换行符
          string  SB = Convert.ToChar(11)+"";//开始符
          string  EB = Convert.ToChar(28)+"";//结束符 char CR;//换行符
          if (strSource.StartsWith(SB))
          {
             try
             {
                 ...
                 StreamReader sr = new StreamReader(@".\Dirui.BF6800.txt",Encoding.Default
);
          ...
          + may.EB + may.CR;
                 ...
                 for (int i = 0; i < strSourceArray.Length; i++)
                 {
                     if (strSourceArray[i].Split(new char[] { '|' })[0].Replace("\n", "").Equals("OBR"))
                     {
                         ...
                         if ("".Equals(strSourceArray[i].Split('|')[7])) { times = DateTime.Now.ToString("yyyyMMddHHmmss"); }
                         else { times = strSourceArray[i].Split('|')[7]; }
                         strTestTime = times.Substring(0, 4) + "-" + times.Substring(5, 2) + "-" + times.Substring(8, 2);
