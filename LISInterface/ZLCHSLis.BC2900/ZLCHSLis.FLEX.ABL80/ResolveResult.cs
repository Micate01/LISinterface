...
namespace ZLCHSLis.FLEX.ABL80
{
    public class ResolveResult : IDataResolve
    {
        ...
        public void ParseResult(string strSource, ref string strResult, ref string strReserved, ref string strCmd)
        {
            ...
            string endStr = Convert.ToChar(0x04) + "";
            ...
            if (strResult.EndsWith(endStr) || strResult.EndsWith("test-end"))
            {
                ...
                try
                {
                    ...
                    string messageTime = analysis[7];
                    string messageID = analysis[10];
                    StreamReader sr = new StreamReader(@".\FLEX.ABL80.txt");
                    ...
                    for (int i = 0; i < strSourceArray.Length; i++)
                    {
                        ...
                        {
                            strSampleNo = strSourceArray[i].Split('|')[3].Split('^')[0];
                        ...
                        if (strSourceArray[i].IndexOf("OBX") != -1)
                        {
                            ...
                            writelog.Write(strDevice, "通道：第 "+i +"次:"+ strSourceArray[i].Split(new char[] { '|' })[3], "log");
