...
namespace ZLCHSLis.DIRUI.N400
{
    public class ResolveResult : IDataResolve
    {
        ...
        public void ParseResult(string strSource, ref string strResult, ref string strReserved, ref string strCmd)
        {
            ...
            if (strSource.EndsWith(c03 + "") || strSource.EndsWith("test-end"))
            {
                ...
                try
                {
                    ...
                    strResult = strResult.Replace("Neg", "-");
                    StreamReader sr = new StreamReader(@".\DIRUI.N400.txt",Encoding.Default);
                    ...
                    for (int i = 0; i < ss.Length; i++)
                    {
                        ...
                        if (ss[i].Trim().IndexOf("Date:") != -1)
                        {

                            strTestTime = ss[i].Split(':')[1];
                            strTestTime = strTestTime.Substring(0,10);
                        }
                        if (ss[i].IndexOf("No.") != -1)
                        {
                        ...
                        for (int j = 0; j < correspondenceArray.Length; j++)
                        {
                            ...
                            {
                                string ItemValue = ss[i].Trim();
                                sBuilder.Append(zlchs + ',' + ItemValue.Split(new char[] { ' ' }, StringSplitOptions.RemoveEmptyEntries)[1] + '|');
                            }
                        }
                    }
