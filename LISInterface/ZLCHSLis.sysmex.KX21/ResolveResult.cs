...
namespace ZLCHSLis.sysmex.KX21
{
    public class ResolveResult : IDataResolve
    {
        ...
        {
            char c20 = Convert.ToChar(0x20);
            string endStr = ""+Convert.ToChar(0x03);
            strResult += strSource;
            if (strResult.EndsWith(endStr) || strResult.EndsWith("end-test"))
            {
                strSource = strResult;
                strResult = "";
                string[] spliteSource = strSource.Split('Z');
                string[] spliteStrSampleNo = spliteSource[0].Split(new char[] { c20 }, StringSplitOptions.RemoveEmptyEntries);
                strSampleNo = spliteStrSampleNo[spliteStrSampleNo.Length - 1];
            }
             try
                    {
