/*************************************
 * 名称：ZLCHSLis.HORRON.H900
 * 功能：电解质
 * 作者：谢天
 * 时间：2018-3-23
 * 通讯类型：串口
 * COM1 波特率:2400   数据位:8   停止位:1   校验:无    控制流:无 
 * 备注:无需应答字符串
 * ***********************************/
using System;
...
namespace ZLCHSLis.HORRON.H900
{
    public class ResolveResult : IDataResolve
    {
        ...
        public void ParseResult(string strSource, ref string strResult, ref string strReserved, ref string strCmd)
        {
            ...
            char c00 = Convert.ToChar(00);
            string endString = "End";
            strResult += strSource;
            if (strSource.EndsWith(endString) || strSource.EndsWith(endString + c00))
            {
                strResult = strResult.Trim(c00);

                writelog.Write(strDevice, "接收数据开始：\r\n " + strResult, "log");
                writelog.Write(strDevice, "接收数据完成 ", "log");

                string[] outer = strResult.Split(new string[] { "End" }, StringSplitOptions.RemoveEmptyEntries);

                for (int iouter = 0; iouter < outer.Length; iouter++)
                {
                   
                    strResult = outer[iouter];
                    writelog.Write(strDevice, "接收数据完成 " + iouter+":" + strResult, "log");
                    try
                    {
                        bool isSampleBar = true;
                        //将接收到的字符串写入到日志文件中
                        writelog.Write(strDevice, iouter+"次接收数据开始：\r\n " + strResult, "log");
                        writelog.Write(strDevice, "接收数据完成 ", "log");

                        StreamReader sr = new StreamReader(@".\HORRON.H900.txt");
                        ...
                        string ChannelCode = "";
                        strTestTime = "20" + ss[3].Trim();
                        strSampleNo = ss[7].Trim();
