...
namespace ZLCHSLis.Diagnosis
{
    public class ResolveResult : IDataResolve
    {
        ...
        {
            string file = "Diagnosis样本重复处理.txt";
            try
            {
                //读取配置文件确定txt文件的路径
                StreamReader sr = new StreamReader(".\\Diagnosis.txt", System.Text.Encoding.GetEncoding("GB2312"));
                string correspondence = sr.ReadLine();
                string configPath = sr.ReadLine();
                sr.Close();
                string[] correspondenceArray = correspondence.Split('|');
                //确定当前日期所产生的文件
                string fileCreateTime = DateTime.Now.ToString("yyyy-MM");
               
                StreamReader srValues = new StreamReader(configPath, System.Text.Encoding.GetEncoding("UTF-8"));
                ...
                for (int iOuter = 1; iOuter < allTexts.Count; iOuter++)
                {
                    string str = "";
                    List<string> TestValues = new List<string>();
                    string[] lines = allTexts[iOuter].Replace('"', ' ').Split(',');
                    strSampleNo = lines[7];
                    strTestTime = lines[1].Split('/')[0];
                    string strName = lines[5].ToUpper();
                    string strValue = lines[9].Replace('↑', ' ').Trim();

                    if (isNumber(strSampleNo) && strTestTime.IndexOf(fileCreateTime)!=-1)
                    {
                        for (int j = 0; j < correspondenceArray.Length; j++)
                        {
                            ...
                            if (strName.Trim().Equals(lis.Trim()))
                            {
                                str = zlchs + ',' + strValue;
                                TestValues.Add(zlchs + ":" + strValue);
                            }
                        }
