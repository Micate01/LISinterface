...
namespace ZLCHSLis.URIT910C  
{
    public class ResolveResult : IDataResolve
    {
        ...
        {     
            ...
            strResult += strSource;
            if (strResult.EndsWith(endStr) || strResult.EndsWith(endStr1) || strResult.EndsWith(endStr2))
            {
               
                try
                {
                    writelog.Write(strDevice, "接收到数据开始： " + strResult, "log");
                    writelog.Write(strDevice, "接收到数据完成： " + strResult, "log");

                    string[] analysis = strResult.Split(new char[]{' ','\t'},StringSplitOptions.RemoveEmptyEntries);
                    strSampleNo = analysis[1];
                    strTestTime = "20"+analysis[2];
                    StringBuilder sBuilder = new StringBuilder();
                    sBuilder.Append("K,"+analysis[4]+"|");
                    sBuilder.Append("Na," + analysis[7] + "|");
                    sBuilder.Append("Cl," + analysis[10] + "|");
                    sBuilder.Append("nCa," + analysis[13] + "|");
                    sBuilder.Append("Ca," + analysis[16] + "|");
                    sBuilder.Append("pH," + analysis[19] + "|");
