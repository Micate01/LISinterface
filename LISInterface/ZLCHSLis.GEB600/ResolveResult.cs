/*************************************
 * 名称：Global GEB600 高尔宝
 * 功能：尿常规
 * 作者：谢天
 * 时间：2017-12-5
 * 通讯类型：串口
 * COM1 波特率:4800   数据位:7   停止位:1   校验:无    控制流:无 
 * 备注:仪器只提供了2400和4800两种波特率
 * ***********************************/
using System;
        ...
        public void ParseResult(string strSource, ref string strResult, ref string strReserved, ref string strCmd)
        {
            ...
            if (strSource.EndsWith(endStr))
            {
                ...
                try
                {
                    ...
                    StreamReader sr = new StreamReader(@".\GEB600.txt",Encoding.Default);
