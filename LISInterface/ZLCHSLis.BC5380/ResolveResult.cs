 ...
 * 作者：谢天
 * 时间：2016-05-31
 * 修改时间：2018-9-17增加了批量处理样本的功能，同时修复了检验时间错误的问题
 * 通讯类型：网络
        ...
        public void ParseResult(string strSource, ref string strResult, ref string strReserved, ref string strCmd)
        {
            ...
            if (result.StartsWith("4"))
            {
                try
                {
                    ...
                    StreamReader sr = new StreamReader(@".\BC5380.txt");
            ...
            {
                strSource = strSource.Replace(may.SB + "", "");
                string[] sbig = strSource.Split(new string[] { "MSH" }, StringSplitOptions.RemoveEmptyEntries);
                for (int ibig = 0; ibig < sbig.Length; ibig++)
                {
                    strSource = sbig[ibig];
                    try
                    {
