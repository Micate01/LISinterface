 ...
 * 时间：2017-4-11
 * 通讯类型：串口
 * COM1 波特率:9600   数据位:8   停止位:1   校验:无    控制流:无   编码GB2312
 * 备注:无需应答字符串
 * ***********************************/
using System;
        ...
        public void ParseResult(string strSource, ref string strResult, ref string strReserved, ref string strCmd)
        {
            ...
            if (strResult.EndsWith(c3D + "" + c00))
            {
                ...
                try
                {
                    ...
                    strTestTime = strSourceArray[6].Replace("日    期: ", "").Trim();
                    strSampleNo = strSourceArray[1].Split(':')[1].Split(' ')[1];
                      string str ="";
                    if (strSourceArray[4].IndexOf("PROG") > -1)
                    {
                        string strTestValue1 = strSourceArray[4].Split(new char[] { ' ' }, StringSplitOptions.RemoveEmptyEntries)[2];
                        str = "PROG," + strTestValue1;
                    }
                    else
                    {
                        string strTestValue2 = strSourceArray[4].Split(new char[] { ' ' }, StringSplitOptions.RemoveEmptyEntries)[2];
                        str = "HCG," + strTestValue2;
                    }
