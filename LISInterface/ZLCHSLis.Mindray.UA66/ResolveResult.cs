...
namespace ZLCHSLis.Mindray.UA66
{
    public class ResolveResult : IDataResolve
    {
        ...
        public void ParseResult(string strSource, ref string strResult, ref string strReserved, ref string strCmd)
        {
            ...
            string endStr=Convert.ToChar(0x20)+""+Convert.ToChar(0x20)+Convert.ToChar(0x20)+Convert.ToChar(0x0D)+Convert.ToChar(0x0A);
            if (strResult.EndsWith(endStr)&&(strResult.IndexOf("ASC")!=-1))
            {
                strSource = strResult;
                ...
                try
                {
                    ...
                    StreamReader sr = new StreamReader(@".\Mindray.UA66.txt");
                    ...
                    for (int i = 0; i < ss.Length; i++)
                    {
                        ...
                        for (int j = 0; j < correspondenceArray.Length; j++)
                        {
                            ...
                            if (ss[i].IndexOf(lis) >= 0)
                            {
                                ...
                                sBuilder.Append(zlchs + ',' + 
                                    temp.Split(new char[] { '\t' }, StringSplitOptions.RemoveEmptyEntries)[1].Trim() + '|');
                            }
                        }
                    }
                    }
