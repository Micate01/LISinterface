    ...
    public class ResolveResult : IDataResolve
    {
        ...
        public void ParseResult(string strSource, ref string strResult, ref string strReserved, ref string strCmd)
        {
            ...
            try
            {
                ...
                string file = "BS2样本号重复处理.txt";
                StreamReader sr = new StreamReader(@".\BS2.txt" ,Encoding.Default);
                ...
                //string sql = "select distinct(bh) as ItemNo  from  tabkhsj   where  jyrq='in_dateTimeName'";
                //sql = sql.Replace("in_dateTimeName", dateTimeName);
                //DataTable dt = OleDBHelper.GetOledbDataTable(sql, connStr);
                string connStr = "Provider=Microsoft.Jet.OLEDB.4.0;Data Source=" + configPath + ";Persist Security Info=False;Jet OLEDB:Database Password=iamysh";
                string sql = "select distinct(bh) as ItemNo  from  tabkhsj   where   jyrq='in_dateTimeName'  or  sjrq= 'in_dateTimeName' ";
                sql = sql.Replace("in_dateTimeName", dateTimeName);
                DataTable dt = OleDBHelper.GetOledbDataTable(sql, connStr);
                ...
                if (dt.Rows.Count > 0)
                    foreach (DataRow item in dt.Rows)
                    {
                        ...
                        string sqlSelect = @"  select *  from tabkhsj where bh='in_strSampleNo' and( jyrq='in_dateTimeName'  or  sjrq= 'in_dateTimeName' )";
