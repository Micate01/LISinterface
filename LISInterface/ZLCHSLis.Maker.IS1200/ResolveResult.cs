...
namespace ZLCHSLis.Maker.IS1200
{
    ...
    public class ResolveResult : IDataResolve
    {
        ...
        public void ParseResult(string strSource, ref string strResult, ref string strReserved, ref string strCmd)
        {
            ...
            try
            {
            string file = "IS1200样本号重复处理.txt";            
            StreamReader sr = new StreamReader(@".\IS1200.txt", System.Text.Encoding.GetEncoding("GB2312"));
            ...
            string sql = "SELECT   distinct(posno) as testNo   from historysampleinfo   where  Format(detectbegintime, 'yyyy-mm-dd') ='" + dateTime + "'";
            ...
            {               
                ...
                string sqlSelect =@"select *  from  historysampleinfo  where  Format(detectbegintime, 'yyyy-mm-dd') ='in_testTime' and  posno='in_testNo'";
                ...
                List<string> TestValues = new List<string>();
                    ...
                    foreach (DataRow subItem in dt2.Rows)
                    {
                        ...
                        string lisTestName = subItem["TestName"].ToString();
