...
namespace ZLCHSLis.Urine.MA4280Kb
{
    public class ResolveResult : IDataResolve
    {
        ...
        public void ParseResult(string strSource, ref string strResult, ref string strReserved, ref string strCmd)
        {
            ...
            try
            {
                strTestTime = DateTime.Parse(strSource.Substring(strSource.IndexOf("Dat") + 4, 10) + " " + strSource.Substring(strSource.IndexOf("Tim") + 4, 8)).ToString("yyyy-MM-dd HH:mm:ss");
            }
            catch
            {
            ...
            /********标本号********/
            strSampleNo = strSource.Substring(strSource.IndexOf("NO:") + 4, 5);
            /****解析结果****/
