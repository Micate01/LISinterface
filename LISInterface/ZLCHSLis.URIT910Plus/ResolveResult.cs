/*************************************
 * 名称：电解质URIT910plus解析程序
 * 功能：电解质解析程序
 * 作者：谢天
 * 时间：2017-03-01
 * 通讯类型：串口
 * 备注:应答符见配置文件URIT910plus.txt  
 * ***********************************/
using System;
...
namespace ZLCHSLis.URIT910Plus  
{
    public class ResolveResult : IDataResolve
    {
        ...
        {       
            strResult += strSource;
            if (strResult.IndexOf("OBX|1|NM|Cl") != -1 && strResult.IndexOf("96.0-106.0|N|||F||||||") >- 1)
            {
                strSource = strResult;
                strResult = "";
                try
                {
                    ...
                    StreamReader sr = new StreamReader(@".\URIT910plus.txt");
