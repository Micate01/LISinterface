...
namespace ZLCHSLis.Rayto.RT6100
{
    ...
    public class ResolveResult : IDataResolve
    {
        ...
        public void ParseResult(string strSource, ref string strResult, ref string strReserved, ref string strCmd)
        {
            ...
            try
            {
                string file = "Rayto.RT-6100样本号重复处理.txt";
                StreamReader sr = new StreamReader(@".\Rayto.RT6100.txt", System.Text.Encoding.GetEncoding("GB2312"));
                ...
                sr.Close();
                string connStr = "Provider=Microsoft.Jet.OLEDB.4.0;Data Source=" + configPath + ";Persist Security Info=False;Jet OLEDB:Database Password=" + pwd;
                string dateTime = DateTime.Now.ToString("yyyy-MM-dd");
                string sql = "SELECT   distinct(sample_id) as testNo   from table_patient   where  Format(pa_test_date, 'yyyy-mm-dd') ='" + dateTime + "'";
                ...
                foreach (DataRow item in dt.Rows)
                {
                    ...
                    string sqlSelect = @"select *  from  table_plate_result  where  Format(testdate, 'yyyy-mm-dd') ='in_testTime' and  sampleid='in_testNo'";
                    ...
                    foreach (DataRow subItem in dt2.Rows)
                    {
                        ...
                        string lisTestName = subItem["itemname"].ToString();
                        string lisTestValue = subItem["resultquantity"].ToString();
