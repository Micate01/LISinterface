        ...
        public void ParseResult(string strSource, ref string strResult, ref string strReserved, ref string strCmd)
        {
            ...
            if (strSource.EndsWith(c03 + "")||strSource.EndsWith("test-end"))
            {
                ...
                try
                {
                    ...
                    StreamReader sr = new StreamReader(@".\GFU180.txt");
                    ...
                    for (int i = 0; i < ss.Length; i++)
                    {
                        if (i == 1)
                        {
                            ...
                            strTestTime = temp[0].Replace(""+Convert.ToChar(0x02),"") + "-" + temp[1] + "-" + temp[2];
                        }
                        if (ss[i].IndexOf("IND") != -1 && ss[i].IndexOf("No.") != -1)
                        {
                            string mm = ss[i].Split('.')[1].Split('-')[0];
                            strSampleNo = mm;
                        }
