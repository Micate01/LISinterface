...
namespace ZLCHSLis.FASCOW3010BX
{
    public class ResolveResult : IDataResolve
    {
        ...
        public void ParseResult(string strSource, ref string strResult, ref string strReserved, ref string strCmd)
        {
            ...
            try
            {
                ...
                string file = "FASCOW3010BX样本重复处理.txt";
                StreamReader sr = new StreamReader(@".\FASCOW3010BX.txt", Encoding.Default);
                ...
                if (!File.Exists(configPath))
                {
                    ...
                    writelog.Write(strDevice,
                        configPath + "没有找到源数据,请检查文件路径", "log");
                ...
                string sql = "select distinct(编号) as ItemNo  from  Record  where Format(日期, 'yyyy-mm-dd')='" + dateTime + "'";
                ...
                if (dt.Rows.Count > 0)
                    foreach (DataRow item in dt.Rows)
                    {
                        ...
                        string sqlSelect = @"select  *  from  Record where    编号=in_编号 and  Format(日期, 'yyyy-mm-dd')=  'in_clrq' ";
                        ...
                        if (dt2.Rows.Count > 0)
                            foreach (DataRow item2 in dt2.Rows)
                            {
                                foreach (var subItem in correspondenceArray)
                                {
                                    ...
                                    string itemValue = item2[lis].ToString();
