 ...
 * 时间：2017-2-20
 * 通讯类型：串口
 * COM1 波特率:19200   数据位:8   停止位:1   校验:无    控制流:无 
 * 备注:无需应答字符串
 * 2017-6-28日修改,解决新妙inr不能出来问题
 * ***********************************/
using System;
...
namespace ZLCHSLis.RAC030
{
    public class ResolveResult : IDataResolve
    {
        ...
        private bool dealStr(string input)
        {
            string[] inputs = input.Split(',');
            if (inputs.Length >= 7)
            {
                if (inputs[inputs.Length - 7].Equals("D-Dimer"))
                    return true;
                else return false;
            }
            else return false;
        ...
        public void ParseResult(string strSource, ref string strResult, ref string strReserved, ref string strCmd)
        {
            ...
            StreamReader sr = new StreamReader(@".\RAC030.txt");
            string regx1 = sr.ReadLine();
            string regx2 = sr.ReadLine();
            sr.Close();   
            strResult += strSource;
            if (strSource.EndsWith("FINISH"))
            {
                #region
                strSource = strResult;
                strResult = "";
                string[] inputs = strSource.Split(new string[] { "R," }, StringSplitOptions.RemoveEmptyEntries);
                ...
                foreach (string item in inputs)
                {
                    if (item.Length > 10)
                    {
                        ...
                        strSampleNo = items[0];
                        if (strSampleNo.Length > 4)
                        {
                            strSampleNo = strSampleNo.Substring(strSampleNo.Length - 4, 4);
                        }


                        strTestTime = items[1];

                        if (regx2.IndexOf(items[10]) > -1)
                        {

                            sBuilder = items[10].ToUpper() + "," + items[14];
                        }

                        else
                        {
                            sBuilder = items[10].ToUpper() + "," + items[11];

                        }

                        if (items[10] == "PT")
                        {
                            sBuilder += "|" + "INR," + items[13];
                        }
            ...
            {
                if (strSource.EndsWith(",0.000,0.000") && dealStr(strSource + strResult))
                {
