 ...
 * 通讯类型：网络
 * 
 * 备注:程序作为客户端，仪器程序作为服务端 仪器端口是5500
 * ***********************************/
using System;
...
namespace ZLCHSLis.Mindray.BC5800
{
    public class ResolveResult : IDataResolve
    {
        ...
        public void ParseResult(string strSource, ref string strResult, ref string strReserved, ref string strCmd)
        {
            ...
            if (strSource.Equals(Convert.ToChar(02) + "")) return;


            else
            {
                strResult += strSource;
                if (strSource.EndsWith(may.CR + "" + may.EB + may.CR))
                {
                    strSource = strResult;
                    strResult = "";
                    ...
                    if (result.StartsWith("5"))
                    {
                        try
                        {
                            ...
                            StreamReader sr = new StreamReader(@".\Mindray.BC5800.txt");
