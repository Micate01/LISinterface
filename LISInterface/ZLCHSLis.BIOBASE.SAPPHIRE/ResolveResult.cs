...
namespace ZLCHSLis.BIOBASE.SAPPHIRE
{
    public class ResolveResult : IDataResolve
    {
        ...
        public void ParseResult(string strSource, ref string strResult, ref string strReserved, ref string strCmd)
        {
            ...
            try
            {
                ...
                string file = "BIOBASE.SAPPHIRE样本重复处理.txt";
                StreamReader sr = new StreamReader(@".\BIOBASE.SAPPHIRE.txt" ,Encoding.Default);
                string correspondence = sr.ReadLine();
                string[] correspondenceArray = correspondence.Split('|');
                string configPath = sr.ReadLine();
                string dateTimeName = "yb" + DateTime.Now.ToString("yyyyMMdd");
                ...
                string sql = "select distinct(ybno) as sampleNo  from  zzjgb";
                ...
                if (dt.Rows.Count > 0)
                    foreach (DataRow item in dt.Rows)
                    {
                        ...
                        strSampleNo = item["sampleNo"].ToString();
                        string sqlSelect = @"select ywjc as ItemName, zzjg1 as ItemValue  from zzjgb   where  ybno=in_sampleNo";
                        sqlSelect = sqlSelect.Replace("in_sampleNo", strSampleNo);
                        ...
                        if (dt2.Rows.Count > 0)
                            foreach (DataRow item1 in dt2.Rows)
                            {
                                ...
                                TestValues.Add(item1["ItemName"].ToString() + ":" + item1["ItemValue"].ToString());
