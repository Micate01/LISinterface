...
namespace ZLCHSLis.Rayto.Hemaray86
{
    public class ResolveResult : IDataResolve
    {
        ...
        {
            char d =Convert.ToChar(0x0d);
            char c=Convert.ToChar(0x1c);
            string endstr = d + "" + c;
            strResult += strSource;
             ...
             + may.EB + may.CR;
                    ...
                    for (int i = 0; i < strSourceArray.Length; i++)
                    {
                        ...
                        if (strSourceArray[i].IndexOf("OBR") != -1)
                        {
                            ...
                            strTestTime = strSourceArray[i].Split('|')[7].Substring(0,10);
                            writelog.Write(strDevice, strSourceArray[i], "log");
                            writelog.Write(strDevice, strSourceArray[i].Split('|')[7], "log");
                        }
                        if (strSourceArray[i].IndexOf("OBX") != -1)
                        {
                            sBuilder.Append(strSourceArray[i].Split('|')[3] + ',' + strSourceArray[i].Split('|')[5] + '|');
