...
namespace ZLCHSLis.WOMO
{
    public class ResolveResult : IDataResolve
    {
        ...
        {
            string file = "WOMO样本号重复处理.txt";
            try
            {
                //读取配置文件确定txt文件的路径
                StreamReader configPathReader = new StreamReader(".\\WOMO.txt", System.Text.Encoding.GetEncoding("GB2312"));
                string configPath = configPathReader.ReadLine();
                configPathReader.Close();
                // System.Threading.Thread.Sleep(3000);
                //确定当前日期所产生的文件
                string fileName = DateTime.Now.ToString("yyyyMMdd").Substring(2, 6);
                fileName += ".txt";
                if (!File.Exists(fileName))
                    return;
                StreamReader sr = new StreamReader(configPath + fileName, System.Text.Encoding.GetEncoding("GB2312"));
                string s;
                int iCount = 0;
                string strSampleNo = "";
                string strTestTime = "";
                ...
                while ((s = sr.ReadLine()) != null)
                {
                    ...
                    if (iCount > 1)
                    {
                        ...
                        strSampleNo = sArray[2];
                        no1 = strSampleNo;
                        strTestTime = sArray[1];
                        time1 = strTestTime;
                        strOperator = sArray[16];
                        string a23 = "";
                        for (int j = 0; j < sArray.Length; j++)
                        {
                            string sTemp = sArray[j];
                            if (sTemp.Equals("")) sTemp = "没有值";

                            if (j != 23)
                            {
                                sBuilder.Append("a" + j + "," + sTemp + '|');
                            }
                            ...
                            if (j == 27)
                            {
                                ...
                                sBuilder.Append("a23" + "," + a23 + '(' + sTemp.Replace("淋球菌", "") + ')' + '|');

                            }
                        }
                        if (Helper.CompareSampleNoAndTime(file, no1, time1, iCount))
                        {
                            continue;
                        }
                        string str = sBuilder.Remove(sBuilder.Length - 1, 1).ToString(); 
