...
namespace ZLCHSLis.URIT8020A
{
    public class ResolveResult : IDataResolve
    {
        ...
        {
            string output = "";
            input = input.Replace(STX + "", "");
            input = input.Split(ETX)[0] + ETX;
        ...
        public void ParseResult(string strSource, ref string strResult, ref string strReserved, ref string strCmd)
        {
            ...
            if (strSource.EndsWith(may.EB + "") || strSource.EndsWith("test-end") || 1==1)
            {
                try
                {
                    ...
                    StreamReader sr = new StreamReader(@".\URIT8020A.txt",Encoding.Default);
