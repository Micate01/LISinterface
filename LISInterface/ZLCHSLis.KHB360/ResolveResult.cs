 ...
 * 作者：谢天
 * 时间：2018-4-9
 * 备注:以后所有的KHB系列产品均加入此样本版本控制内容
 * 修改：
 * 修改内容：把配置文件模式取消更改为读取原始字符
 * ***********************************/
using System;
...
namespace ZLCHSLis.KHB360
{
    public class ResolveResult : IDataResolve
    {
        ...
        public void ParseResult(string strSource, ref string strResult, ref string strReserved, ref string strCmd)
        {
            ...
            try
            {
                string file = "KHB360样本号重复处理.txt";
                StreamReader sr = new StreamReader(@".\KHB360.txt", Encoding.Default);
