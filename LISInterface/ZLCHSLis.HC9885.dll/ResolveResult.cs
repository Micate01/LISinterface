 ...
 * 时间：2017-4-13
 * 通讯类型：串口
 * COM1 若为：VER10.01 或 VER11.01,则虚拟串口参数为：
 * 波特率：1200 校验位：None 数据位：7 停止位：1
 * 若为：Ver 2.x.xxx (x-xxxx) 字样的版本号，则虚拟串口参数为：
 * 波特率：115200,校验位：None 数据位：8 停止位：1
 * 备注:无需应答字符串
 * ***********************************/
using System;
...
namespace ZLCHSLis.HC9885
{
    public class ResolveResult : IDataResolve
    {
        ...
        {
            string  endStr = Convert.ToChar(0x0a)+""+Convert.ToChar(0x0d);
            strResult += strSource;        
            if (strResult.EndsWith(endStr)||strResult.EndsWith("test-end"))
            {
                strSource = strResult;
                strResult = "";
                try
                {
                    ...
                    string[] strSourceArray = strSource.Split(new char[] { ' ' },StringSplitOptions.RemoveEmptyEntries);                   
                    strTestTime = strSourceArray[0].Split('/')[0]+"-"+strSourceArray[0].Split('/')[1]+"-"+strSourceArray[0].Split('/')[2];
                    strSampleNo = strSourceArray[2];
                    string str = "";
                    str += "K," + strSourceArray[3] + "|";
                    str += "Na," + strSourceArray[4] + "|";
                    str += "Cl," + strSourceArray[5] + "|";
                    str += "iCa," + strSourceArray[6] + "|";
                    str += "TCa," + strSourceArray[7] + "|";
                    str += "pH," + strSourceArray[8];   
