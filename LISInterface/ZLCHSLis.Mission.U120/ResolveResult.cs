...
namespace ZLCHSLis.Mission.U120
{
    public class ResolveResult : IDataResolve
    {
        ...
        public void ParseResult(string strSource, ref string strResult, ref string strReserved, ref string strCmd)
        {
            ...
            if (strResult.EndsWith(endStr) || strResult.EndsWith("test_end"))
            {
                strSource = strResult;
                strResult = "";
                StreamReader sr = new StreamReader(@".\Mission.U120.txt", Encoding.Default);
                ...
                try
                {
                    ...
                    {
                        if (strSourceArray[i].IndexOf("Date:") != -1)
                        {
                            strTestTime = strSourceArray[i].Trim().Replace("Date:", "").Split(new char[] { ' ' }, StringSplitOptions.None)[1];
                        }
                        if (strSourceArray[i].IndexOf("No.") != -1)
                        {
                            strSampleNo = strSourceArray[i].Trim().Replace("No.", "").Trim();
                        }
                        for (int j = 0; j < correspondenceArray.Length; j++)
                        {
                            ...
                            if (strSourceArray[i].ToUpper().IndexOf(lis) != -1)
                            {
                                string ItemValue = strSourceArray[i].Trim().Split(new char[] { ' ' }, StringSplitOptions.RemoveEmptyEntries)[1];
