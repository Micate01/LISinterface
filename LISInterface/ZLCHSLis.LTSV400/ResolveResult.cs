...
namespace ZLCHSLis.LTSV400
{
    public class ResolveResult : IDataResolve
    {
        ...
        public void ParseResult(string strSource, ref string strResult, ref string strReserved, ref string strCmd)
        {
            ...
            try
            {
                string file = "LTSV400样本号重复处理.txt";
                StreamReader sr = new StreamReader(@".\LTSV400.txt",Encoding.Default);
                string correspondence = sr.ReadLine();
                string[] correspondenceArray = correspondence.Split('|');
                string configPath = sr.ReadLine();
                string dateTimeName = DateTime.Now.ToString("yyyy-MM-dd");
                ...
                string sql = @"select distinct(a.barcode) as ItemNo  from
                                patientInfos a, Results b
                        where a.seq=b.seq and   format (SubmitDate,'yyyy-MM-dd')='in_dateTimeName'";
                sql = sql.Replace("in_dateTimeName", dateTimeName);
                ...
                if (dt.Rows.Count > 0)
                    foreach (DataRow item in dt.Rows)
                    {
                        ...
                        string sqlSelect = @"select a.barcode as ItemNo ,itemname ,itemresult as itemValue from                     patientInfos a, Results b
                        where a.seq=b.seq  and   format (SubmitDate,'yyyy-MM-dd')='in_dateTimeName' and a.barcode='in_strSampleNo' ";
                        sqlSelect = sqlSelect.Replace("in_dateTimeName", dateTimeName);
                        ...
                        if (dt2.Rows.Count > 0)
                            foreach (DataRow item2 in dt2.Rows)
                            {
                                ...
                                {
                                    string lis = subItem.Split(',')[0].Trim().ToUpper();
                                    string zlchs = subItem.Split(',')[1].Trim().ToUpper();
                                    string itemName = item2["itemname"].ToString().ToUpper();
                                    string itemValue = item2["itemValue"].ToString().ToUpper();
                                    if (itemName == lis)
                                    {
