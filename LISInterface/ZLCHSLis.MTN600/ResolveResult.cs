        ...
        public void ParseResult(string strSource, ref string strResult, ref string strReserved, ref string strCmd)
        {
            ...
            if (strSource.EndsWith(endStr))
            {
                ...
                try
                {
                    ...
                    StreamReader sr = new StreamReader(@".\MTN600.txt", Encoding.Default);
                    ...
                    {
                        if (ss[i].IndexOf("NO.") >= 0 || ss[i].IndexOf("No.") >= 0)
                        {
                            strSampleNo = ss[i].Split(new char[] { ' ' }, StringSplitOptions.RemoveEmptyEntries)[0].Replace("NO.", "");
                            strSampleNo = ss[i].Split(new char[] { ' ' }, StringSplitOptions.RemoveEmptyEntries)[1];
                        }
                        if (ss[i].IndexOf("Date") >= 0)
                        {
