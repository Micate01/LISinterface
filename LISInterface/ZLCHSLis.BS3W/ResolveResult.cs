...
namespace ZLCHSLis.BS3W
{
    ...
    public class ResolveResult : IDataResolve
    {
        ...
        public void ParseResult(string strSource, ref string strResult, ref string strReserved, ref string strCmd)
        {
            ...
            //配置文件中读取密码,查询语句          
            //读取配置文件
            string file = "BS3W样本号重复处理.txt";
            init();
            StreamReader sr = new StreamReader(@".\BS3W.txt", System.Text.Encoding.GetEncoding("GB2312"));
            ...
            string dateTimeFormat2 = DateTime.Now.ToString("yyyy-MM-dd");

            strTestTime = dateTime;         
            string sql = "select distinct(bh) as swatch  from  tabkhsj   where    Format(jyrq, 'yyyy.mm.dd')='" + dateTime + "'";
            ...
            foreach (DataRow item in dt.Rows)
            {
                ...
                string sqlSelect = @"SELECT *    FROM  tabkhsj   where Format(jyrq, 'yyyy.mm.dd')='" + dateTime + "'       and  bh ='" + bbh + "'";
                ...
                try
                {
                    ...
                    foreach (DataRow item1 in dt2.Rows)
                    {
                        for (int j = 0; j < correspondenceArray.Length; j++)
                        {
                            ...
                            strTestTime = dateTimeFormat2;
                            strSampleNo = bbh;
                            time1 = strTestTime;
                            no1 = bbh;
                            string lis = correspondenceArray[j].Split(',')[0].Trim().ToUpper();
                            string zlchs = correspondenceArray[j].Split(',')[1].Trim().ToUpper();
                            string lisValue = item1[lis].ToString().Trim().ToUpper();
                            TestValues.Add(lis.ToString() + ":" + lisValue.ToString());
                            sBuilder.Append(zlchs.ToUpper() + ',' + lisValue.ToString().Trim() + '|');
