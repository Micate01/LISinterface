...
namespace ZLCHSLis.Omlipo
{
    public class ResolveResult : IDataResolve
    {
        ...
        private Mindray may = new Mindray();
        ...
        public void ParseResult(string strSource, ref string strResult, ref string strReserved, ref string strCmd)
        {
            ...
            if (strSource.EndsWith(may.EB + "" + may.CR) || strSource.EndsWith("test-end"))
            {
                strResult = strResult + strSource;
                string result = may.sendQCKXCG(strResult, strDevice, strDeviceID);
                #region  如果是质控到话
                if (result.StartsWith("4"))
                {
                 ...
                 + may.EB + may.CR;
                        ...
                        for (int i = 0; i < strSourceArray.Length; i++)
                        {
                            ...
                            {
                                strSampleNo = "900" + strSourceArray[i].Split('|')[3];
                ...
                else if (result.StartsWith("5"))
                {
                 ...
                 + may.EB + may.CR;
                        ...
                        string times = "";
                        string[] correspondenceArray = correspondence.Split('|');
                        string[] strSourceArray = strResult.Split(new char[] { may.CR }, StringSplitOptions.RemoveEmptyEntries);
                        for (int i = 0; i < strSourceArray.Length; i++)
                        {

                            string[] strSourceArray1 = strSourceArray[i].Split('|');
                            if (strSourceArray1.Length > 1)
                            {
                                if (strSourceArray[i].Split(new char[] { '|' })[0].Replace("\n", "").Equals("OBR"))
                                {
